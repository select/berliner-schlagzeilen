...
let lastDate = '';
const newTweets = fs.readdirSync(`${config.dataDir}/img`)
	...
	.map(file => {
	    ...
	    status: `${m.format('DD.MM.YYYY')} http://zefys.staatsbibliothek-berlin.de/kalender/auswahl/date/${m.format('YYYY-MM-DD')}/27971740 #1918LIVE`,
...
fs.writeFileSync(`${config.dataDir}/tweets.json`, JSON.stringify(allTweets, null, 2));
process.stdout.write(`\nwrote file ${config.dataDir}/tweets.json\n`);
