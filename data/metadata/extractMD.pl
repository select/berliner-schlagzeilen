...
# G�n�ration des m�tadonn�es de documents METS/ALTO :
# param�tre 1 : dossier des documents � traiter
# param�tres suivants : formats de sortie
...
# r�pertoire de stockage des m�tadonn�es extraites
$OUT = "STATS";
if(-d $OUT){
...
# r�pertoire de stockage des documents
$DOCS = "DOCS";

# nbre de documents trait�s
$nbDoc=0;
...
# table de hashage m�tadonn�es/valeurs
%hash = ();	
...
# ----------------------                                             
# r�cup�ration des m�tadonn�es biblio                    
...
sub lireMD {
  ...
  #$t -> parsefile($ficMETS);  
  #$t -> purge(); # d�charger le contenu pars�
  
  # extraire par regexp (plus rapide)   
  open my $fh, '<', $ficMETS or die "Impossible d'ouvrir : $ficMETS !";
  if ($articles==0) {
  	local $/;
  	( $titre ) = <$fh> =~ m/\<mods:title\>(.+)\<\/mods:title\>/; 
 	  seek $fh, 0, 0;  
  	( $date ) = <$fh> =~ m/\>(.+)\<\/mods:dateIssued/; 
  	  seek $fh, 0, 0;
  	( $Volume ) = <$fh> =~ m/\>(.+)\<\/mods:number/; 
 	  seek $fh, 0, 0;  
  	( $Issue ) = <$fh> =~ m/\>(.+)\<\/mods:number/; 
  	  seek $fh, 0, 0;
  	$hash{"titre"} = $titre;	
	$hash{"date"} = $date;
	$hash{"Jahrgang"} = $Volume;	
	$hash{"Nummer"} = $Issue;
...
sub lireMDALTO {
  ...
  while (my $line = <$fh>) { 	
   ...
   $composed++ if $line =~ /<ComposedBlock/;
   $ills++ if $line =~ /<Illustration/;
   $graph++ if $line =~ /<GraphicalElement/;
  } 
   
  $hash{$numPage."_mots"} = $mots;  
  $hash{$numPage."_blocsTexte"} = $textes;
  $hash{$numPage."_blocsComposed"} = $composed;
  $hash{$numPage."_blocsIllustration"} = $ills;
  $hash{$numPage."_blocsGraphicalElement"} = $graph;
...
sub ecrireMD {
  ...
  if ($format eq "csv") {
    ...
    print {$fh} "</contenus></analyseAlto>\n";
  	...
  	print {$fh} "   \"Jahrgang\": \"".$hash{"Jahrgang"}."\",\n";
  	print {$fh} "   \"Nummer\": \"".$hash{"Nummer"}."\",\n";
    ...
    for($p = 1; $p <= $hash{"pages"}; $p++) {      	
      ...
      print {$fh} " \"blocGraphics\": ".$hash{$p."_blocsGraphicalElement"}.",";
      print {$fh} " \"blocComposed\": ".$hash{$p."_blocsComposed"}.",";
...
# ----------------------                            
# calcul d'un ark  � partir d'un ID de document BnF 
...
# calcul d'un caract�re de controle ark � partir d'un nom ark
